...
<html lang="pt-br">
    <head>
        ...
        <style>
            img{display: block; margin: 2px;  border: 2px solid #ccc; width: 700px; }
        </style>
    </head>
    <body>
        <?php
        for ($i = 0; $i < count($recibos); $i++) {
            echo '<img src="' . BASE_URL . '/' . $recibos[$i] . '">';
        }
        ?>
    </body>
</html>
...
ob_end_clean();
$mpdf = new \Mpdf\Mpdf(['mode' => 'utf-8', 'margin_left' => 5, 'margin_right' => 2, 'margin_top' => 2, 'margin_bottom' => 2, 'margin_header' => 2, 'margin_footer' => 2]);
$mpdf->WriteHTML($html);
